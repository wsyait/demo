        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl
                v-for="(skuSaleAttr, index) in skuInfo.skuSaleAttrValueList"
                :key="skuSaleAttr.id"
              >
                ...
                <dd
                  ...
                  class="active"
                  @click="
                    changeActive(skuSaleAttr, skuInfo.skuSaleAttrValueList)
                  "
                >
                  <!--        v-for="(
                    skuSaleAttrValue, index
                  ) in skuSaleAttr.skuSaleAttrValueList"
            ...
            <div class="cartWrap">
              ...
              <div class="add">
                <!-- 以前咱们的路由跳转，从A路由跳转到B路由，这里在加入购物车之前，进行路由跳转
                ，发请求，把购买的产品的信息通过请求的形式通知服务器，服务器进行相应的存储 -->
...
export default {
  ...
  computed: {
    ...
    //给子组件的数据
    skuImageList() {
      //如果服务器的数据没有回来，skuInfo这个对象是空对象，会使得skuInfo后面的打点的变成undefined
  ...
  methods: {
    ...
    //加入购物车的回调函数
    async addShopcar() {
      //1.发请求--将产品参数加入到数据库（通知服务器）
      //当前这里是派发一个action，也向服务器发请求
      //判断加入购物车是成功了还是失败了，进行相应的操作
      //下面这个代码说白了，就是调用仓库中的addOrUpdateShopCart，这个方法加上asyc，返回的一定是一个promise
      //要么成功、要么失败，我们需要知道这次请求是成功还是失败
      try {
        //成功干什么
        ...
        //下面这种手段路由跳转一级传递参数可以
        //本地存储（H5-超文本语言的第五次更改-新增-本地存储{持久化-5M}和会话存储{并非持久化}）
        //一些简单的数据如SKUnum，通过query形式给路由组件传递过去
        //产品信息的数据（比较复杂skuInfo），通过会话存储（不持久化，会话结束数据再消失）
        //本地存储和会话存储，一般存储的是字符串
